...
export const createCour = asyncHandler(async (req, res) => {
  ...
  res.status(201).send(new ApiResponse(201, { cours: newCour }, 'Cour created successfully ✅'));
});
...
export const updateCour = asyncHandler(async (req, res) => {
  ...
  if (!existingCour) {
    res.status(404).send(new ApiError(401, 'Cour not found ❌'));
    return;
  }

  const updatedCour = await Cour.findByIdAndUpdate(courId, {
    ...req.body,
    prix: parseInt(req.body.prix),
    teacher: new mongoose.Types.ObjectId('5ae4cedafe51b99a8ff1b022'),
    subject: new mongoose.Types.ObjectId(req.body.subject),
  ...
  res.status(200).send(new ApiResponse(200, { updatedCour }, 'Cour updated successfully ✅'));
});
...
export const deleteCour = asyncHandler(async (req, res) => {
  ...
  res.status(200).send(new ApiResponse(200, {}, 'Cour deleted successfully ❌'));
});

export const getAllCours = asyncHandler(async (req, res) => {
  const cours = await Cour.find();

  res.status(200).send(new ApiResponse(200, { cours }, 'Cours retrieved successfully ✅'));
});
